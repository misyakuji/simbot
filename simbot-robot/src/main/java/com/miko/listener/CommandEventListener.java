...
import com.miko.ai.service.ArkModelService;
...
public class CommandEventListener {
    ...
    /**
     * AI模型服务，用于获取和管理AI模型相关功能
     */
    private final ArkModelService arkModelService;
    /**
     * 消息中断管理器
     */
    private final MessageInterruptManager messageInterruptManager;
    ...
    public void friendMsgCmdEvent(OneBotFriendMessageEvent event) {
        // 1. 获取当前模型和模型列表
        List<String> modelList = arkModelService.getModels();
        String currentModel = arkModelService.getCurrentModel();
        ...
        // 4. 标记中断：防止其他监听器继续处理同一条消息，
        // 将当前事件ID标记为已处理，避免重复响应或冲突
        messageInterruptManager.setInterrupt(event.getId().toString());
    }
